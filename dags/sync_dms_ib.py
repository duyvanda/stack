...
def insert():
    dmssql = \
    f"""
    select CONCAT(BranchID,BatNbr) as pk from {from_tb} where cast (Crtd_DateTime as date ) >= '{date_ms15}'
    """
    ...
    bqsql = \
    f"""
    delete from biteam.{table_name} where date(crtd_datetime) >= '{date_ms15}' and pk in {del_tp}
    """
    print(bqsql)
